...
test("dawn voyage preserves wind rules across undo, witch action and daybreak", async () => {
  ...
  assert.equal((await post(`${roomPath}/exile-record`, { ...auth, noExile: true })).status, 200);
...
test("remote first-night flows include each board's identity confirmation steps", async () => {
  ...
  for (const [boardId, expectedSteps] of Object.entries(expectedConfirmations)) {
    ...
    if (boardId === "treasure_master" && started.body.room.assignments.some((assignment) => assignment.roleId === "masked_man")) {
      assert.ok(stepIds.includes("masked_man_confirm"), `${boardId} missing masked_man_confirm`);
    }
  }
});
