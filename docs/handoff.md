...
> 当前实现、发布基线与待验收事项请先阅读 [`current-status.md`](./current-status.md)。截至 2026-08-29，线上 `main` 基线为 `e5ce7f8`，自动化测试为 36/36；下文除“命名储备”外均为历史交接快照，不作为当前规则或发布状态依据。

## 命名储备（2026-08-10，候选，未定稿）

- 雾海回响：保留为后续赛季候选名。
- 逆序长夜：保留为后续赛季候选名。
- 烬月归潮：保留为后续赛季候选名。
- 月色未尽，余烬再燃：保留为赛季海报或宣传文案候选。
- 惊风折烛：保留为后续赛季候选名。

## 历史状态与已确认问题（2026-07-29）

当前代码基线为 `main` 的 `a6e84ea`，当前工作分支为 `codex/role-card-prompts`，仅比 `main` 多出角色卡绘图提示文档提交 `dfc0e36`。本工作区尚未提交的流程、安全与“唯邻是从”改动已通过 `npm.cmd test` 29/29 与 Node / Pages Function / 前端脚本语法检查；该结果不代表 Cloudflare D1 并发或浏览器多端流程已经验收。

本轮已完成下列流程修复，并补充了相应的服务端回归测试；仍须在真实浏览器和线上 Pages/D1 环境验收：

1. **无法官空座错误（已修复，待多端验收）**：服务端拒绝没有玩家标识的选座请求，并在每次读取时规范化无归属座位；前端不会把没有归属人的座位渲染为已占。已覆盖 7 号空座的回归测试。
2. **有法官开夜条件（已修复）**：JUDGE 模式可跳过未记录的票型、放逐、天亮死亡和死亡技能待办，直接开下一夜；SYSTEM 模式仍保留“先记录当天最终出局结果”等约束。
3. **无法官白神公开（已修复）**：白痴被放逐后仅在该时点公开播报“X号玩家出局，翻牌为白神。”；已覆盖精确文案和公开身份记录。
4. **无法官夜间推进与播报（已修复，待浏览器验收）**：每次有效行动推进后，服务端对下一步骤设置 10 秒门控并拒绝提前行动；公共设备在门控结束后自动播报下一流程 3 次。默认 10 秒配置已用本地服务实测，自动测试通过环境变量将等待缩短为 0 秒。
5. **安全阻断项（部分已修复，待线上验收）**：公开房间响应已剥离 `clientId` / `userId`，新客户端使用加密安全随机标识；法官口令连续失败 5 次会锁定 5 分钟。Cloudflare D1 的乐观并发写入（冲突时返回 409）尚需线上实际并发验收。

文档对账结论：README 曾漏列“曙光航纪”；`CLAUDE.md`、本文件的历史快照和 `rule-data-structure-v0.1.md` 含有不同阶段的旧状态。后续以“代码 + 本节的明确待办”为当前状态，规则变更仍需回写到产品规则和对应版型文档。

新增版型：第 7 个可选版型“唯邻是从”已加入网页、本地服务、Cloudflare Function 与平衡发牌器，详见 `docs/board-follow-neighbor-v0.1.md`。发牌保留所有连坐结构，仅对三狼相邻范围内神职少于 2 人的候选加软惩罚；本地 100 局验收应在对话交付前记录实际均值。
