...
const SYSTEM_STEP_ANNOUNCEMENT_DELAY_MS = Math.max(0, Number(process.env.SYSTEM_STEP_ANNOUNCEMENT_DELAY_MS || 10000));
...
function createControllerTransferCode() {
...
function createNightSteps(boardId, night, room = null) {
  ...
  if (room?.treasureMaskedFinalNight?.status === "PENDING") {
    ...
    if (dreamerAlive) common.push({ id: "dreamer_dream", actor: "dreamer", label: "摄梦人选择摄梦目标", targetCount: 1, allowSkip: false });
    common.push({ id: "treasure_mask_final_kill", actor: "treasure_mask_final", label: "盗宝蒙面狼进行濒死最终刀", targetCount: 1, allowSkip: false });
    return common;
  }
  if (boardId === "follow_neighbor") {
    const witchStep = createWitchStep(room);
    common.push({ id: "guard_guard", actor: "guard", label: "守卫选择守护目标", targetCount: 1, allowSkip: true });
    if (firstNight) common.push({ id: "puppet_select", actor: "wolf_team", label: "狼人和狼王选择傀儡目标", targetCount: 1, allowSkip: false, allowedSeats: getFollowNeighborPuppetSeats(room) });
    common.push({ id: "wolves_kill", actor: "wolf_team", label: "狼人和狼王选择击杀目标", targetCount: 1, allowSkip: true });
...
async function handleApi(request, response, url) {
  ...
  if (body.expectedRevision !== undefined && Number(body.expectedRevision) !== Number(room.revision || 0)) {
    return sendError(response, 409, "房间状态已更新，请刷新后重试");
  }

  if (target.action === "controller-transfer-start") {
    if (room.mode !== "SYSTEM" || !isController(room, judgeToken)) return sendError(response, 403, "只有当前控制设备可以发起转让");
    room.controllerTransferCode = createControllerTransferCode();
  ...
  if (target.action === "controller-transfer-claim") {
    if (room.mode !== "SYSTEM") return sendError(response, 400, "只有无法官房间可以转让控制权");
    if (!clientId) return sendError(response, 400, "缺少设备标识");
    if (!room.controllerTransferCode || Date.now() > Number(room.controllerTransferExpiresAt || 0)) return sendError(response, 400, "转让码已过期，请由当前控制设备重新生成");
    if (String(body.controllerTransferCode || "") !== room.controllerTransferCode) return sendError(response, 403, "转让码错误");
    if ((room.seats || []).some((seat) => seat.clientId === clientId)) return sendError(response, 400, "已入座的玩家设备不能接管公共控制权");
    const transferPhase = room.phase;
    room.judgeToken = crypto.randomBytes(18).toString("hex");
  ...
  if (target.action === "judge-claim") {
    ...
    if (judgeClaimLocked(room)) return sendError(response, 429, "法官口令尝试过多，请 5 分钟后再试");
  ...
  if (target.action === "seat") {
    if (!clientId) return sendError(response, 400, "缺少玩家标识");
    ...
    if (targetSeat.occupied && targetSeat.clientId && targetSeat.clientId !== clientId) {
  ...
  if (target.action === "deal") {
    ...
    if (room.seats.some((seat) => !seat.occupied || !seat.clientId)) return sendError(response, 400, "需要 12 人满座才能发牌");
  ...
  if (target.action === "system-timeout-skip") {
    if (room.mode !== "SYSTEM" || !isController(room, judgeToken)) return sendError(response, 403, "只有公共控制设备可以处理超时");
    ...
    if (!room.systemStepDeadlineAt || Date.now() < Number(room.systemStepDeadlineAt)) return sendError(response, 400, "当前步骤尚未超时");
    const actionRecord = {
      night: room.night,
      stepId: step.id,
      label: step.label,
  ...
  if (target.action === "night-action") {
    ...
    const seat = room.seats.find((item) => item.clientId === clientId);
    const assignment = seat ? room.assignments.find((item) => item.seat === seat.seat) : null;
    if (room.mode === "SYSTEM" && !isSystemAnnouncementReady(room)) return sendError(response, 400, "正在等待下一流程播报");
